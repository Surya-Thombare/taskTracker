...
const timerHandler = (io, socket) => {
  ...
  socket.on('timer:start', async (data, callback) => {
    try {
      ...
      callback({ success: false, message: 'Failed to start timer' });
    }
  });
  ...
  socket.on('timer:complete', async (data, callback) => {
    try {
      ...
      if (!timerId) {
        return callback({ success: false, message: 'No active timer found for this task' });
      }
      ...
      if (!timer || !timer.isActive) {
        return callback({ success: false, message: 'Timer not found or already completed' });
      }
      ...
      callback({ success: false, message: 'Failed to complete timer' });
    }
  });
  ...
  socket.on('timer:get:active', async (callback) => {
    try {
      ...
      if (!timer) {
        return callback({ success: true, timer: null });
      }
      ...
      callback({ success: false, message: 'Failed to get active timer' });
    }
  });
};
