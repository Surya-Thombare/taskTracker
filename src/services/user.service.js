...
const updateProfile = async (user, profileData) => {
  const { firstName, lastName, bio, avatar } = profileData;
...
const getTimerHistory = async (userId, options) => {
  ...
  // Count total timers
  const totalTimers = await Timer.countDocuments({ user: userId, isCompleted: true });

  // Get timers
  const timers = await Timer.find({ user: userId, isCompleted: true })
    .populate('task', 'title')
    .populate('group', 'name')
    .sort({ endTime: -1 })
    .skip(skip)
...
const getDashboardStats = async (userId) => {
  ...
  const pendingTasks = await Task.countDocuments({
    $or: [
      ...
      { assignees: userId, status: 'pending' }
    ]
  });
  ...
  const inProgressTasks = await Task.countDocuments({
    $or: [
      { creator: userId, status: 'in-progress' },
      { assignees: userId, status: 'in-progress' }
    ]
  });
  ...
  const recentCompletedTasks = await Task.countDocuments({
    $or: [
      { creator: userId, status: 'completed', completedAt: { $gte: thirtyDaysAgo } },
      { 'completedBy.user': userId, 'completedBy.completedAt': { $gte: thirtyDaysAgo } }
    ]
  });
