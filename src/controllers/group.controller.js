...
const addMember = async (req, res) => {
  try {
    ...
    if (error.message === 'Only group leaders can add members' ||
...
const removeMember = async (req, res) => {
  try {
    ...
    if (error.message.includes('Only group leaders can remove members') ||
      error.message === 'User is not a member of this group' ||
      error.message === 'Cannot remove a leader. Demote to member first') {
      return errorResponse(
        res,
        httpStatus.BAD_REQUEST,
        error.message
...
const promoteMember = async (req, res) => {
  try {
    ...
    return successResponse(
      res,
      httpStatus.OK,
      'Member promoted to leader successfully',
      { user }
    );
  } catch (error) {
    ...
    if (error.message === 'Only group leaders can promote members' ||
      error.message === 'User is not a member of this group' ||
      error.message === 'User is already a leader') {
      return errorResponse(
        res,
        httpStatus.BAD_REQUEST,
        error.message
...
const demoteLeader = async (req, res) => {
  try {
    const { groupId, leaderId } = req.params;
    ...
    return successResponse(
      res,
      httpStatus.OK,
      'Leader demoted to member successfully',
      { user }
    );
  } catch (error) {
    ...
    if (error.message === 'Only group leaders can demote leaders' ||
      error.message === 'User is not a leader' ||
      error.message === 'Cannot demote the group creator' ||
