...
const startTimer = async (req, res) => {
  try {
    ...
    return successResponse(res, httpStatus.CREATED, 'Timer started successfully', {
      timer: timerData
    });
  } catch (error) {
    ...
    if (error.message === 'You already have an active timer. Complete it before starting a new one.' || error.message.includes('Cannot start timer for a') || error.message === 'You do not have permission to work on this task') {
      return errorResponse(res, httpStatus.BAD_REQUEST, error.message);
    }
    return errorResponse(res, httpStatus.INTERNAL_SERVER_ERROR, 'Failed to start timer');
  }
};
...
const completeTimer = async (req, res) => {
  try {
    ...
    return successResponse(res, httpStatus.OK, 'Timer completed successfully', {
      timer: timerData
    });
  } catch (error) {
    ...
    return errorResponse(res, httpStatus.INTERNAL_SERVER_ERROR, 'Failed to complete timer');
  }
};
...
const getActiveTimer = async (req, res) => {
  try {
    ...
    return successResponse(res, httpStatus.OK, timer ? 'Active timer retrieved successfully' : 'No active timer found', {
      timer
    });
  } catch (error) {
    logger.error('Error retrieving active timer:', error);
    return errorResponse(res, httpStatus.INTERNAL_SERVER_ERROR, 'Failed to retrieve active timer');
  }
};
...
const listTaskTimers = async (req, res) => {
  try {
    ...
    return successResponse(res, httpStatus.OK, 'Task timers retrieved successfully', {
    ...
    return errorResponse(res, httpStatus.INTERNAL_SERVER_ERROR, 'Failed to list task timers');
  }
};
module.exports = {
  startTimer,
