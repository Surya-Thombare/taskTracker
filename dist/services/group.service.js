...
const getGroupDetails = async (groupId, userId) => {
  // Find group
  const group = await Group.findById(groupId).populate('creator', 'firstName lastName avatar').populate('leaders', 'firstName lastName avatar').populate('members', 'firstName lastName avatar');
  if (!group) {
...
const promoteMember = async (groupId, memberId, leaderId) => {
  ...
  const promotedUser = await User.findById(memberId).select('firstName lastName avatar');
  return {
...
const demoteLeader = async (groupId, leaderId, actorId) => {
  ...
  group.leaders = group.leaders.filter(leader => leader.toString() !== leaderId.toString());
  ...
  const demotedUser = await User.findById(leaderId).select('firstName lastName avatar');
  return {
    _id: demotedUser._id,
...
const listUserGroups = async userId => {
  ...
  }).select('name description isPublic avatar creator leaders members completedTasks totalTasks createdAt lastActive').populate('creator', 'firstName lastName').sort({
    lastActive: -1
  });
