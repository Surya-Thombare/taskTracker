...
const generateGroupLeaderboard = async (groupId, options = {}) => {
  ...
  const leaderboard = users.map(user => {
    ...
    const completionRate = stats.tasksCompleted > 0 ? Math.round(stats.tasksCompletedOnTime / stats.tasksCompleted * 100) : 0;
    return {
      user: {
        _id: user._id,
...
const generateGlobalLeaderboard = async (options = {}) => {
  ...
  const leaderboard = timerStats.map(stat => {
    ...
    if (!user) return null;
    const completionRate = stat.tasksCompleted > 0 ? Math.round(stat.tasksCompletedOnTime / stat.tasksCompleted * 100) : 0;
    return {
      user: {
        _id: user._id,
        name: `${user.firstName} ${user.lastName}`,
